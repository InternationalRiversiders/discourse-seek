...
旧版收藏存于访问者浏览器的 `eating:favorites`，不在旧数据库中。`script/export_favorites.html` 必须在**旧站同源**提供给用户，导出后由用户在新版收藏页导入。插件不能跨域读取旧浏览器存储；旧站正式切换前需安排这个入口。
