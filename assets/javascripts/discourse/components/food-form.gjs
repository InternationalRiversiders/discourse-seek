...
export default class extends Component {
  ...
  @action async upload(e) {
    ...
    try {
      ...
      if (files.length + this.mediaIds.length > (this.args.form.max_images || 6)) { throw new Error("上传图片数量超过限制"); }
      this.args.dirty?.();
      for (const file of files) {
        const data = new FormData();
        data.append("file", file);
        const result = await ajax("/food/upload", {
          type: "POST",
          data,
          processData: false,
          contentType: false,
  ...
  @action async submit(event) {
    ...
    for (const field of this.args.form.fields) {
      if (["checks", "existing_images"].includes(field.type)) { values[field.name] = formData.getAll(field.name); }
