...
export default class extends Component {
  ...
  <template>
    <main class="food-native" data-view={{this.data.view}} aria-busy={{if this.busy "true" "false"}} {{this.mount}}>
      ...
      {{#if this.isCatalog}}
        <section class="food-discovery">
          {{#if (eq this.data.view "discover")}}<div class="food-discovery-heading"><div><h2>今天吃什么？</h2><p>{{this.data.stats.shops}} 家店铺 · {{this.data.stats.comments}} 条校友点评</p></div><button class="btn btn-flat" {{on "click" this.shuffle}}>{{dIcon "rotate"}} 换一批</button></div><div class="food-modes">{{#each this.data.modes as |mode|}}<button class="btn {{if (eq (if this.data.q.mode this.data.q.mode 'recommend') mode.id) 'is-selected'}}" {{on "click" (fn this.setQuery "mode" mode.id)}}>{{mode.label}}</button>{{/each}}</div>{{/if}}
          <div class="food-areas">{{#each this.data.areas as |area|}}<button class="btn btn-flat {{if (eq (if this.data.q.area this.data.q.area '全部') area.name) 'is-selected'}}" {{on "click" (fn this.setQuery "area" area.name)}}>{{area.name}} <small>{{area.count}}</small></button>{{/each}}</div>
          <form class="food-search" {{on "submit" this.search}}><input type="search" name="q" value={{this.data.q.q}} placeholder="搜店名、地址或想吃的菜" aria-label="搜索店铺" /><select name="category" aria-label="分类"><option value="">全部分类</option>{{#each this.data.categories as |category|}}<option value={{category}} selected={{eq category this.data.q.category}}>{{category}}</option>{{/each}}</select><select name="price" aria-label="人均价格"><option value="">全部价格</option><option value="1" selected={{eq this.data.q.price "1"}}>10 元以下</option><option value="2" selected={{eq this.data.q.price "2"}}>10–20 元</option><option value="3" selected={{eq this.data.q.price "3"}}>20–30 元</option><option value="4" selected={{eq this.data.q.price "4"}}>30–50 元</option><option value="5" selected={{eq this.data.q.price "5"}}>50 元以上</option></select><select name="sort" aria-label="排序"><option value="">默认排序</option><option value="rating-desc" selected={{eq this.data.q.sort "rating-desc"}}>评分优先</option><option value="price-asc" selected={{eq this.data.q.sort "price-asc"}}>价格从低到高</option><option value="price-desc" selected={{eq this.data.q.sort "price-desc"}}>价格从高到低</option><option value="newest" selected={{eq this.data.q.sort "newest"}}>最新收录</option></select><select name="min_rating" aria-label="最低评分"><option value="">全部评分</option><option value="4" selected={{eq this.data.q.min_rating "4"}}>4 星以上</option><option value="3" selected={{eq this.data.q.min_rating "3"}}>3 星以上</option><option value="2" selected={{eq this.data.q.min_rating "2"}}>2 星以上</option></select><select name="status" aria-label="营业状态"><option value="">全部状态</option><option value="open" selected={{eq this.data.q.status "open"}}>营业中</option><option value="closed" selected={{eq this.data.q.status "closed"}}>暂停营业</option></select><input type="hidden" name="area" value={{this.data.q.area}} /><input type="hidden" name="mode" value={{this.data.q.mode}} /><button type="submit" class="btn btn-primary">{{dIcon "magnifying-glass"}} 搜索</button></form>
        </section>
        {{#if (eq this.data.view "favorites")}}<p class="food-muted">{{#if this.data.member}}收藏已与论坛账号同步。旧版收藏保存在原浏览器，请先<a href="https://food.river-side.cc/export-favorites" target="_blank" rel="noopener noreferrer">导出旧版收藏</a>，再在下方导入 JSON。{{else}}收藏保存在当前浏览器；登录论坛后可以使用账号收藏。{{/if}}</p>{{/if}}
        <div class="food-grid">{{#each this.shops key="id" as |shop|}}<Shop @shop={{shop}} @visit={{this.visit}} @favorite={{this.favorite}} />{{else}}<div class="food-empty">{{dIcon "utensils"}}<h3>这里还没有店铺</h3><p>试试其他筛选，或推荐一家你喜欢的店。</p></div>{{/each}}</div>
      {{/if}}
      {{#if this.shop}}
        <a class="food-back" href="/food?view=shops" {{on "click" this.visit}}>← 全部店铺</a>
        <section class="food-detail-head"><div><div class="food-eyebrow">{{this.shop.area}} · {{this.shop.category}}</div><h2>{{this.shop.name}}</h2><div class="food-detail-meta"><span class="food-rating">{{dIcon "star"}} {{if this.shop.rating this.shop.rating "暂无评分"}}</span><span>{{this.shop.price}}</span><span>{{this.shop.comments_count}} 条点评</span><span>{{this.shop.dishes_count}} 道推荐菜</span></div><p>{{dIcon "location-dot"}} {{if this.shop.street this.shop.street "地址待完善"}}</p>{{#if (eq this.shop.business_status "closed")}}<p class="food-banner">暂停营业 · {{this.shop.status_reason}}</p>{{/if}}</div><div class="food-actions"><button class="btn {{if this.shop.favorite 'is-active'}}" {{on "click" (fn this.favorite this.shop)}}>{{dIcon "heart"}} {{if this.shop.favorite "已收藏" "收藏"}}</button><button class="btn" {{on "click" this.share}}>{{dIcon "link"}} 分享</button><button class="btn" {{on "click" this.poster}}>{{dIcon "download"}} 分享卡片</button>{{#if this.data.can_write}}<a class="btn" href={{link (hash view="edit" id=this.shop.id)}} {{on "click" this.visit}}>{{dIcon "pen-to-square"}} 完善资料</a>{{/if}}{{#if this.data.can_manage}}<button class="btn" {{on "click" (fn this.moderate this.shop)}}>管理门店</button>{{/if}}</div></section>
      {{/if}}
      ...
      {{#if this.shop}}
        ...
        {{#if (eq this.data.part "history")}}<div class="food-history">{{#each this.data.history as |item|}}<p><strong>{{item.author.name}}</strong> {{item.body}} <DRelativeDate @date={{item.created_at}} /></p>{{else}}<p class="food-empty">暂无共建记录。</p>{{/each}}</div>{{/if}}
      ...
      {{#if (eq this.data.view "about")}}
        ...
        {{#if this.data.contributors.length}}<section class="food-contributors" aria-label="贡献榜"><h2>贡献榜 <small>{{this.data.contributors.length}} 位贡献者</small></h2><ol>{{#each this.data.contributors key="id" as |contributor|}}<li><span class="food-contributor-rank">{{contributor.rank}}</span><div>{{#if contributor.author.url}}<a href={{contributor.author.url}}>{{contributor.author.name}}</a>{{else}}<strong>{{contributor.author.name}}</strong>{{/if}}<p>贡献 {{contributor.shop_count}} 家店铺</p></div>{{#if (eq contributor.rank 1)}}<span class="food-contributor-first">TOP 1</span>{{/if}}</li>{{/each}}</ol></section>{{/if}}
      {{/if}}
      ...
      {{#if this.data.member}}<footer class="food-footer"><a href="/food/my-data">导出我在觅电的数据</a></footer>{{/if}}
