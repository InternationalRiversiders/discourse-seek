import { foodRequests } from "../lib/food-request-queue";
...
export default class extends Component {
  ...
  @action async navigate(q,event,fromHistory=false){
    ...
    const version=++this.version;this.navigationRequest?.cancel();this.busy=true;this.error="";this.dialogForm=null;this.dirty=false;
    const search=new URLSearchParams(q).toString();
    const request=this.navigationRequest=foodRequests.enqueue(signal=>{
      let xhr;
      const abort=()=>xhr?.abort();
      signal.addEventListener("abort",abort,{once:true});
      return ajax("/food/state.json?"+search,{beforeSend:jqXHR=>{xhr=jqXHR;if(signal.aborted)return false;}}).finally(()=>signal.removeEventListener("abort",abort));
    },{priority:10,delay:200});
    try{const result=await request.promise;if(version!==this.version||this.isDestroying)return;this.snapshot=result;if(!fromHistory&&location.search!=="?"+search)history.pushState({},"",link(q));requestAnimationFrame(()=>{if(this.isDestroying||version!==this.version)return;if(q.reply)document.getElementById(`food-reply-${q.reply}`)?.scrollIntoView({block:"center"});else scrollTo({top:0,behavior:"instant"});});}
    catch(e){if(version===this.version&&!this.isDestroying&&e.name!=="AbortError")this.error=extractError(e);}finally{if(version===this.version&&!this.isDestroying)this.busy=false;}
  }
  willDestroy(){super.willDestroy(...arguments);this.version++;this.navigationRequest?.cancel();}
  ...
  <template>
    <main class="food-native" data-view={{this.data.view}} aria-busy={{if this.busy "true" "false"}} {{this.mount}}>
      ...
      {{#if this.shop}}
        {{#if (eq this.data.part "overview")}}<section class="food-overview"><div><h3>关于这家店</h3><p class="food-body">{{if this.shop.body this.shop.body "介绍还待校友补充。"}}</p>{{#if this.shop.review}}<blockquote class="food-body">{{this.shop.review}}</blockquote>{{/if}}<div class="food-tags">{{#each this.shop.review_tags as |tag|}}<span>{{tag.tag}} {{tag.count}}</span>{{/each}}</div>{{#if this.shop.source_url}}<a href={{this.shop.source_url}} target="_blank" rel="noopener noreferrer">查看原始来源 ↗</a>{{/if}}{{#if this.shop.latitude}}<p class="food-muted">位置：{{this.shop.latitude}}, {{this.shop.longitude}}</p>{{/if}}{{#if this.shop.missing_media_count}}<p class="food-muted">{{this.shop.missing_media_count}} 张历史图片已缺失</p>{{/if}}</div><div class="food-photos">{{#each this.shop.images as |url|}}<button {{on "click" (fn this.photo url)}} aria-label="放大门店照片"><img {{queuedImage url}} alt="门店照片" loading="lazy" /></button>{{/each}}</div></section><h3>同一区域，再逛逛</h3><div class="food-grid food-related" {{cardMasonry ".food-shop"}}>{{#each this.data.related key="id" as |shop|}}<Shop @shop={{shop}} @visit={{this.visit}} @favorite={{this.favorite}} />{{/each}}</div>{{/if}}
        {{#if (eq this.data.part "photos")}}{{#each this.data.gallery as |group|}}<h3>{{group.label}}</h3><div class="food-photos is-gallery">{{#each group.images as |url|}}<button {{on "click" (fn this.photo url)}} aria-label="放大图片"><img {{queuedImage url}} alt={{group.label}} loading="lazy" /></button>{{else}}<p class="food-muted">还没有这类照片。</p>{{/each}}</div>{{/each}}{{/if}}
      ...
      {{#each this.data.proposals as |proposal|}}<article class="food-proposal"><header><h3>{{proposal.title}}</h3><span>{{statusText proposal.status}}</span></header><p><ForumUser @user={{proposal.author}} @name={{proposal.author.name}} /> · {{proposal.reason}} <DRelativeDate @date={{proposal.created_at}} /></p>{{#if proposal.review_reason}}<p>审核说明：{{proposal.review_reason}}</p>{{/if}}{{#if proposal.stale}}<p class="food-banner">这份申请基于旧资料，请逐项核对当前内容。</p>{{/if}}<details><summary>查看修改对照</summary><div class="food-table-wrap"><table><thead><tr><th>字段</th><th>修改前</th><th>申请修改</th><th>当前资料</th></tr></thead><tbody>{{#each proposal.diffs as |diff|}}<tr class={{if diff.conflict "food-conflict"}}><th>{{fieldName diff.field}}{{#if diff.conflict}} · 有冲突{{/if}}</th><td>{{display diff.before}}</td><td>{{display diff.after}}</td><td>{{display diff.current}}</td></tr>{{/each}}</tbody></table></div></details><div class="food-photos">{{#each proposal.images as |url|}}<button {{on "click" (fn this.photo url)}} aria-label="放大申请图片"><img {{queuedImage url}} alt="申请图片" /></button>{{/each}}</div>{{#if proposal.missing_media_count}}<p class="food-muted">{{proposal.missing_media_count}} 张历史图片缺失</p>{{/if}}{{#if (eq this.data.view "admin")}}{{#unless this.data.readonly}}{{#if (eq proposal.status "pending")}}<div class="food-actions"><button class="btn btn-primary" {{on "click" (fn this.review proposal "approve")}}>采纳</button><button class="btn" {{on "click" (fn this.review proposal "reject")}}>不采纳</button></div>{{/if}}{{/unless}}{{/if}}</article>{{/each}}
      ...
      {{#if this.photoUrl}}<div class="food-overlay" role="dialog" aria-modal="true" aria-label="查看图片"><button class="btn food-photo-close" {{on "click" this.closePhoto}}>关闭图片</button><img class="food-lightbox" {{queuedImage this.photoUrl}} alt="放大的美食照片" /></div>{{/if}}
    </main>
  </template>
}
