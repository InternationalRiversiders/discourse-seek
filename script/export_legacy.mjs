...
const models=["User", "Shop", "Dish", "DishLike", "ShopEditSubmission", "Comment", "CommentLike", "ShopSubmission", "SiteConfig", "AboutComment", "AboutCommentLike"];
const payload={format:'riverside-community-v1',project:'seek',exported_at:new Date().toISOString(),tables:{},media:[]};
try {
  ...
  try {
    if(names.size) {
      ...
      for(const name of names) {
        if(path.basename(name)!==name) { throw new Error('Invalid media name'); }
        let source;
        try { source=await fs.realpath(path.join(sourceRoot,name)); } catch(e) {
          if(e.code==='ENOENT' && process.env.FOOD_EXPORT_ALLOW_MISSING_MEDIA==='1') { payload.media.push({name,missing:true});continue; }
          throw e;
        }
